...
const initialState = {
  ...
  integrationHealthMetrics: null as HealthMetricsResponse | null,
  integrationHealthLoading: false,
  integrationHealthError: null as string | null,
  integrationConnectionHealth: [] as IntegrationHealth[],
  integrationConnectionHealthLoading: false,
  integrationConnectionHealthError: null as string | null,
  oauthLoading: false,
  oauthError: null as string | null,
};

export const createIntegrationSlice: StateCreator<IntegrationSlice> = (set, get) => ({
  ...initialState,
  ...
  // Integration Health
  fetchIntegrationHealthMetrics: async (connectionId) => {
    set({ integrationHealthLoading: true, integrationHealthError: null });
    try {
      const metrics = await api.apiGetHealthMetrics(connectionId);
      set({ integrationHealthMetrics: metrics, integrationHealthLoading: false, integrationHealthError: null });
    } catch (error) {
      set({ integrationHealthLoading: false, integrationHealthError: error instanceof Error ? error.message : 'Failed to fetch health metrics' });
    }
  },

  fetchIntegrationConnectionHealth: async (connectionId) => {
    set({ integrationConnectionHealthLoading: true, integrationConnectionHealthError: null });
    try {
      const health = await api.apiGetConnectionHealth(connectionId);
      set({ integrationConnectionHealth: health, integrationConnectionHealthLoading: false, integrationConnectionHealthError: null });
    } catch (error) {
      set({ integrationConnectionHealthLoading: false, integrationConnectionHealthError: error instanceof Error ? error.message : 'Failed to fetch connection health' });
    }
  },
